...
def synthetic_population(T, rm, n0, v_min, v_max, 
                         ...
                         vd, va, R_reff,
                         speed_resolution=100, angle_resolution=90, B_resolution = 50, dr=0.1, 
...
#    b_arr = np.linspace(-np.pi/2, np.pi/2, angle_resolution//2)
    ...
    N_added = 0
    if T > 0:
        B_arr = np.zeros((speed_resolution, B_resolution))
        ...
        if (velocity_components is not None) or (precomputed_p_vlb is not None):
            p_vlb_f = p_vlb[..., np.newaxis] * np.ones_like(v_f)
        else:
        ...
        Flux_element = n0_SI * p_vlb_f * v_f * (2*np.pi*B_f*dB_f) * dv * du * dl / np.cos(b_f)
        
        total_flux = np.sum(Flux_element)
        
        F_flat = Flux_element.ravel()
        P = F_flat / F_flat.sum()   # verovatnoća svake tačke
        CDF = np.cumsum(P)
        ...
        # uzorci
        v_sample = v_arr[v_idx]
        l_sample = l_arr[l_idx]
        b_sample = b_arr[b_idx]
        B_sample = B_arr[v_idx, B_idx]
...
# ============================================================================= 
    ...
    if len(d)>1: 
        ...
        x=np.random.random(total_number+N_added)*(N_ref[-1]-N_ref[0])+N_ref[0] 
        D_s=np.zeros_like(x)
...
# =============================================================================
        ...
        for i in tqdm(range(len(new_sizes)), desc="3/5: Longitude of interstellar velocity vector"):
            ...
            if (velocity_components is not None) or (precomputed_p_vlb is not None):
                ...
                p_lb__vs = fast_spherical_kde(pts)
            else:
...
# =============================================================================
        ...
        for i in tqdm(range(len(new_sizes)), desc="4/5: Latitude of interstellar velocity vector"):
            ...
            if (velocity_components is not None) or (precomputed_p_vlb is not None):
                ...
                p_b__vs__ls = fast_spherical_kde(pts)
            else:
