...
def synthetic_population(T, rm, n0, v_min, v_max, 
                         u_Sun, v_Sun, w_Sun, 
                         sigma_vx, sigma_vy, sigma_vz, 
                         vd, va, R_reff,
                         speed_resolution=50, angle_resolution=60, B_resolution = 50, dr=0.1, 
                         d_ref=1000, d=[], alpha=[], velocity_components=None, precomputed_p_vlb=None):
    ...
    if velocity_components is not None:
        print("Using Data-Driven Velocity Distribution")
        U_data, V_data, W_data = velocity_components
        values = np.vstack([U_data, V_data, W_data])
    ...
    r_arr=np.linspace(r_min, r_max, r_resolution, dtype=np.float32)

    n0_SI = np.float32(n_total / au**3)

    # definicija mreže
    v_arr = np.linspace(v_min, v_max, speed_resolution, dtype=np.float32)
    l_arr = np.linspace(0, 2*np.pi, angle_resolution, dtype=np.float32)
     
#    b_arr = np.linspace(-np.pi/2, np.pi/2, angle_resolution//2)
    u_arr = np.linspace(-1, 1, angle_resolution//2, dtype=np.float32)
    b_arr = np.arcsin(u_arr)
    
    l_mesh, b_mesh = np.meshgrid(l_arr, b_arr)
    l_mesh = l_mesh.astype(np.float32)
    b_mesh = b_mesh.astype(np.float32)
    
    # making 3D arrays for v, l, b 
    # doing it here so we can then do the heavy kde math just once for both T=0 and T>0
    ind=np.mgrid[0:len(v_arr), 0:len(l_arr), 0:len(b_arr)]
    v_base=v_arr[ind[0]]
    l_base=l_arr[ind[1]]
    b_base=b_arr[ind[2]]
    
    fast_spherical_kde = None
    if precomputed_p_vlb is not None:
        p_vlb = precomputed_p_vlb
        fast_spherical_kde = RegularGridInterpolator((v_arr, l_arr, b_arr), p_vlb, bounds_error=False, fill_value=0.0)
    
    elif velocity_components is not None:
        print("Building Fast Spherical KDE Lookup Table...")
        ...
        # compute log-likelihoods in cartesian space
        ...
        # transform back to spherical: p(vx, vy, vz) to p(v, l, b)
        p_vlb = pdf_values.reshape(v_base.shape) * (v_base**2) * np.cos(b_base)
        
        # save the pre-computed 3D probability cube as a fast lookup table for the rest of the simulation
        # it's spherical so no conversions are needed in the generation loops
        fast_spherical_kde = RegularGridInterpolator((v_arr, l_arr, b_arr), p_vlb, bounds_error=False, fill_value=0.0)
    
    else:
        # probability density distribution w.r.t. magnitude and direction of intrstellar velocities (at infinity), Eqs. 14 and 15
        p_vlb = p_v_l_b(v_base, l_base, b_base, sigma_vx, -u_Sun, sigma_vy, -v_Sun, sigma_vz, -w_Sun, vd, va)

    N_added = 0
    if T > 0:
        B_arr = np.zeros((speed_resolution, B_resolution), dtype=np.float32)
        
        for i, v in enumerate(v_arr):
            B_max_of_v = rm * au * np.sqrt(1 + 2*mu / (v**2 * rm * au))
            B_arr[i] = np.linspace(0, B_max_of_v, B_resolution)
        
        # ΔB
        dB = np.diff(B_arr, axis=1)
        dB = np.hstack([dB, dB[:,-1:]])  # shape: (speed_resolution, B_resolution)
        
        # mreža indeksa
        ind_f = np.mgrid[0:len(v_arr), 0:len(l_arr), 0:len(b_arr), 0:B_resolution]
        
        v_f = v_arr[ind_f[0]]
        l_f = l_arr[ind_f[1]]
        b_f = b_arr[ind_f[2]]
        B_f = B_arr[ind_f[0], ind_f[3]]
        dB_f = dB[ind_f[0], ind_f[3]]  # ΔB po tački
        
        # since p(v,l,b) is independent of B, we bypass recalculating the KDE 
        # and we just broadcast the 3D p_vlb array into the 4th dimension if T>0
        if (velocity_components is not None) or (precomputed_p_vlb is not None):
            p_vlb_f = np.broadcast_to(p_vlb[..., np.newaxis], v_f.shape)
        else:
            p_vlb_f = p_v_l_b(v_f, l_f, b_f, sigma_vx, -u_Sun, sigma_vy, -v_Sun, sigma_vz, -w_Sun, vd, va)
        
        # Δv, Δl, Δb
        dv = np.float32(v_arr[1] - v_arr[0])
        dl = np.float32(l_arr[1] - l_arr[0])
        du = np.float32(u_arr[1] - u_arr[0])
        
        Flux_element = n0_SI * p_vlb_f * v_f * (np.float32(2*np.pi)*B_f*dB_f) * dv * du * dl / np.cos(b_f)
        
        total_flux = np.sum(Flux_element)
        
        F_flat = Flux_element.ravel()
        P = np.float64(F_flat) / np.float64(F_flat.sum()) # verovatnoća svake tačke

        CDF = np.cumsum(P)
        
        N = int(total_flux * T * y2s)
        rrr = np.random.random(N)
        indices = np.searchsorted(CDF, rrr)
        
        # vraćanje indeksa u 4D mrežu
        v_idx, l_idx, b_idx, B_idx = np.unravel_index(indices, Flux_element.shape)
        
        # uzorci
        v_sample = v_arr[v_idx].astype(np.float64)
        l_sample = l_arr[l_idx].astype(np.float64)
        b_sample = b_arr[b_idx].astype(np.float64)
        B_sample = B_arr[v_idx, B_idx].astype(np.float64)
        
        N_added = len(v_sample)
    ...
    # preimenujem ovo kako bi kod dole ostao identičan  
    v = v_base
    l = l_base
    b = b_base
    
    # marginal distribution of magnitudes interstellar velocities
    p_v=simps(simps(p_vlb, b_arr), l_arr).astype(np.float32)
...
#        new_sizes=[size]
    ...
    for i in tqdm(range(len(new_sizes)), desc="1/5: Heliocentric distance"):
        ...
        p_rvlb=np.zeros([new_sizes[i], np.shape(v)[0], np.shape(v)[1], np.shape(v)[2]], dtype=np.float32)
        ...
        p_r[indices[i]:indices[i+1]]=simps(simps(simps(p_rvlb, b_arr), l_arr),v_arr).astype(np.float32)
...
# ============================================================================= 
    ...
    if len(d)>1: 
        ...
        #x=np.linspace(N_ref[0], N_ref[-1], total_number+N_added) # uniform sample which is transformed using the Inverse Transform Sampling method
        x=np.random.random(total_number+N_added).astype(np.float32) * (N_ref[-1]-N_ref[0])+N_ref[0] 
        D_s=np.zeros_like(x, dtype=np.float32)
...
# ============================================================================= 
        ...
        p__rs__vs=np.zeros(total_number, dtype=np.float32) # marginal w.r.t l,b,B, conditional w.r.t. rs, vs
        for i in tqdm(range(len(new_sizes)), desc="2/5: Interstellar velocity"):
            
            p_v_cdf=np.zeros([new_sizes[i], speed_resolution], dtype=np.float32) 
            v_vec=np.ones([new_sizes[i], speed_resolution], dtype=np.float32) 
            v_vec=np.multiply(v_arr, v_vec, dtype=np.float32)
            
            r_vec=np.ones([new_sizes[i], speed_resolution], dtype=np.float32) 
            ...
            uv=np.random.rand(new_sizes[i]).astype(np.float32) # random number for inverting interstellar velocity
...
# =============================================================================       
        ...
        uB=np.random.rand(total_number).astype(np.float32) # random number for inverting interstellar impact parameter
...
# =============================================================================
        ...
        for i in tqdm(range(len(new_sizes)), desc="3/5: Longitude of interstellar velocity vector"):
            ...
            vec=np.ones([new_sizes[i],  int(angle_resolution/2), angle_resolution], dtype=np.float32) 
            ...
            if (velocity_components is not None) or (precomputed_p_vlb is not None):
                pts = np.stack((vs_vec, l_vec, b_vec), axis=-1)
                p_lb__vs = fast_spherical_kde(pts).astype(np.float32)
            else:
            ...
            ul=np.random.rand(new_sizes[i]).astype(np.float32) # random number for inverting longitude of IS velocity vector
            for j in range(new_sizes[i]):
                ...
                ls[i*new_size+j] = spl.roots()[0]

            del vec, vs_vec, Bs_vec, rs_vec, l_vec, b_vec, p_lb__vs, p_l__rs__vs__Bs
            gc.collect()
...
# =============================================================================
        ...
        for i in tqdm(range(len(new_sizes)), desc="4/5: Latitude of interstellar velocity vector"):
            ...
            if (velocity_components is not None) or (precomputed_p_vlb is not None):
                pts = np.stack((vs_vec, ls_vec, b_vec), axis=-1)
                p_b__vs__ls = fast_spherical_kde(pts).astype(np.float32)
            else:
            ...
            ub=np.random.rand(new_sizes[i]).astype(np.float32)
            for j in range(new_sizes[i]):
                ...
                bs[i*new_size+j] = spl.roots()[0]
            
            del ls_vec, vs_vec, Bs_vec, rs_vec, b_vec, p_b__vs__ls, p_b__rs__vs__Bs__ls
            gc.collect()
...
# =============================================================================
        ...
        if T > 0:
            ...
            inc_add = np.zeros(len(bs))
            node_add = np.zeros(len(bs))
            ...
            for i in tqdm(range(len(v_sample)), desc='6/6: Conversion to orbital elements dodatnih'):
                ...
                # inclination
                inc_add[i]=np.arccos(np.dot(orbital_plane_normal, np.array([0,0,1])))
            
                # unit vector toward ascending node
                node = np.cross(np.array([0, 0, 1]), orbital_plane_normal)
                
                # longitude of ascending node
                node_add[i] = np.arctan2(node[1], node[0])
                ...
                # argument of perihelion
                argument_add[i] = np.arccos(np.dot(nn, e_vector) / np.linalg.norm(nn) / np.linalg.norm(e_vector))
                
                if e_vector[2] < 0:
                    argument_add[i] = 2 * np.pi - argument_add[i]
            
            # semi-major axis       
            semi_major_axis_add=-mu/vs**2
            # perihelion distance
            q_add=semi_major_axis_add+np.sqrt(semi_major_axis_add**2+Bs**2)
            # eccentricity
            e_add=np.sqrt(1+Bs**2/semi_major_axis_add**2)
        
            node_add=np.mod(node_add,2*np.pi)
        
            # random choice time of entry
            t_add = np.random.random(len(e_add)) * T * y2s
            
                    
            H_limit = np.arccosh(1/e_add * (1 - rm/(semi_major_axis_add/au)))
            M_limit = e_add * np.sinh(H_limit) - H_limit
            
            mm = np.sqrt(-mu/semi_major_axis_add**3)
            
            M_add = -M_limit - mm * t_add
            
            f_add = np.zeros_like(M_add)
            
            for i in range(len(M_add)):
                E_add = kepler(e_add[i], M_add[i])
                f_add[i] = ecc2true(E_add, e_add[i])
            
            r_add = semi_major_axis_add * (1-e_add**2) / (1 + e_add * np.cos(f_add))
        
            if len(d)>1:
                return(np.concatenate([q_s, q_add])/au, np.concatenate([e_s, e_add]),
                       np.concatenate([f_s, f_add]), np.concatenate([inc_s, inc_add]),
                       np.concatenate([node_s, node_add]), np.concatenate([argument_s, argument_add]), D_s)
            else:
                return(np.concatenate([q_s, q_add])/au, np.concatenate([e_s, e_add]),
                       np.concatenate([f_s, f_add]), np.concatenate([inc_s, inc_add]),
                       np.concatenate([node_s, node_add]), np.concatenate([argument_s, argument_add]))
