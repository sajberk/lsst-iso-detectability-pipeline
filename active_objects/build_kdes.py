...
def build_static_kde(U, V, W, v_arr, l_arr, b_arr):
    ...
    v_base = v_arr[ind[0]].astype(np.float32)
    l_base = l_arr[ind[1]].astype(np.float32)
    b_base = b_arr[ind[2]].astype(np.float32)
    ...
    pdf_values = np.exp(log_pdf).astype(np.float32)
    
    # Convert back to spherical probability density: p(v, l, b)
    p_vlb = (pdf_values.reshape(v_base.shape) * (v_base**2) * np.cos(b_base)).astype(np.float32)
    return p_vlb
...
def build_all_kdes():
    ...
    required_files = ["kde_grid_thin_32.npz", "kde_grid_thick_32.npz", "kde_grid_halo_32.npz"]
    ...
    vmin_thin = np.float32(np.min(speeds_thin) * 0.95)
    vmax_thin = np.float32(np.max(speeds_thin) * 1.05)
    v_arr_thin = np.linspace(vmin_thin, vmax_thin, SPEED_RESOLUTION, dtype=np.float32)
    ...
    np.savez("kde_grid_thin_32.npz", grid=p_vlb_thin, v_min=vmin_thin, v_max=vmax_thin)
    ...
    vmin_thick = np.float32(np.min(speeds_thick) * 0.95)
    vmax_thick = np.float32(np.max(speeds_thick) * 1.05)
    v_arr_thick = np.linspace(vmin_thick, vmax_thick, SPEED_RESOLUTION, dtype=np.float32)
    ...
    np.savez("kde_grid_thick_32.npz", grid=p_vlb_thick, v_min=vmin_thick, v_max=vmax_thick)
    
    # HALO DISK
    U_halo, V_halo, W_halo = pops_200['HALO']
    speeds_halo = np.sqrt(U_halo**2 + V_halo**2 + W_halo**2)
    vmin_halo = np.float32(np.min(speeds_halo) * 0.95)
    vmax_halo = np.float32(np.max(speeds_halo) * 1.05)
    v_arr_halo = np.linspace(vmin_halo, vmax_halo, SPEED_RESOLUTION, dtype=np.float32)
    ...
    np.savez("kde_grid_halo_32.npz", grid=p_vlb_halo, v_min=vmin_halo, v_max=vmax_halo)
