...
def filter_and_convert(q, e, inc, Omega, omega, f, D_m, albedo, H_c, is_comet=False, hc_max_comet=9999):
    """ Filters out objects that won't be observable and calculates tp and H for sorcha"""

    hc_max_ast = max_hc_distance_asteroid(D_m, albedo, 24.5)

    if is_comet:
        # we take the maximum of the coma fade-out and the bare nucleus fade-out.
        hc_max = np.maximum(hc_max_comet, hc_max_ast)
    else:
        hc_max = hc_max_ast
    ...
    H_out = absolute_magnitude_asteroid(D_out, albedo)
    H_c_out = H_c2[selection2]
    ...
    return q_out, e_out, inc_out, Omega_out, omega_out, tp_out, D_out, H_out, H_c_out
